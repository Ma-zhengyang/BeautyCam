...
public class RainFlake {
    ...
    private RandomUtil mRandom;

    private RainFlake(RandomUtil random, Line line, float increment, float flakeSize, Paint paint) {
        mRandom = random;
        mLine = line;
    ...
    public static RainFlake create(int width, int height, Paint paint) {
        ...
        int[] nline = random.getLine(width, height);
    ...
    private void drawLine(Canvas canvas, int width, int height) {
        ...
        Log.d(TAG, "drawLine: "+mLine.x1+", "+ mLine.y1 + ", " +mLine.x2+ ", "+mLine.y2);
    ...
    private void resetLine(int width, int height) {
        int[] nline = mRandom.getLine(width, height);
        mLine.x1 = nline[0];
        mLine.y1 = nline[1];
        mLine.x2 = nline[2];
        mLine.y2 = nline[3];
